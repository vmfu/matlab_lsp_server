...
- [ ] 1.1.7 Коммит: `git commit -m "feat(project): create basic project structure"`
- [ ] 1.1.8 Пуш: `git push origin master`
- [ ] 1.1.9 Суммаризация: Краткое описание выполненной работы и изменений
- [ ] 1.1.10 Переход к 1.2 и исполнение TODOs

### 1.2 Реализация менеджера конфигурации
- [ ] **1.2.1 Анализ задачи**: Изучение `.matlab-lsprc.json`, проектирование `ConfigManager` с использованием pydantic для валидации конфигурации. **Использовать**: context7 MCP для документации pydantic
- [ ] 1.2.2 Создание модели конфигурации в `src/utils/config.py` с pydantic
- [ ] 1.2.3 Реализация `ConfigManager` для чтения `.matlab-lsprc.json` и переменных окружения
- [ ] 1.2.4 Добавление валидации путей к MATLAB и других настроек
- [ ] 1.2.5 Интеграция `ConfigManager` в `server.py`
- [ ] 1.2.6 Тест: Проверка чтения конфигурации из файла и env переменных
- [ ] 1.2.7 Отметка выполнения в TODO.md (поставить ✅ в 1.2.1-1.2.5)
...
- [ ] **1.3.1 Анализ задачи**: Изучение требований к логированию для LSP сервера, проектирование `get_logger()` с использованием colorlog. **Использовать**: context7 MCP для документации colorlog и Python logging
- [ ] 1.3.2 Создание модуля `src/utils/logging.py` с настройкой цветного логирования
- [ ] 1.3.3 Добавление уровней логирования (DEBUG, INFO, WARNING, ERROR)
- [ ] 1.3.4 Интеграция логирования в `server.py`
- [ ] 1.3.5 Тест: Проверка вывода логов с разными уровнями
- [ ] 1.3.6 Отметка выполнения в TODO.md (поставить ✅ в 1.3.1-1.3.4)
- [ ] 1.3.7 Коммит: `git commit -m "feat(logging): implement colored logging system"`
...
- [ ] **1.4.1 Анализ задачи**: Изучение паттерна обработчиков LSP методов, проектирование `BaseHandler` для всех LSP хендлеров. **Использовать**: context7 MCP для документации pygls и lsprotocol
- [ ] 1.4.2 Создание `src/handlers/base.py` с базовым классом `BaseHandler`
- [ ] 1.4.3 Определение абстрактного метода `handle()` для конкретных реализаций
- [ ] 1.4.4 Добавление свойств `method_name` для идентификации LSP метода
- [ ] 1.4.5 Тест: Создание тестового хендлера и проверка его структуры
- [ ] 1.4.6 Отметка выполнения в TODO.md (поставить ✅ в 1.4.1-1.4.4)
- [ ] 1.4.7 Коммит: `git commit -m "feat(handlers): implement BaseHandler for LSP methods"`
...
- [ ] 2.2.2 Реализация извлечения определений классов (classdef/end)
- [ ] 2.2.3 Реализация извлечения свойств классов
- [ ] 2.2.4 Реализация извлечения методов классов
- [ ] 2.2.5 Реализация извлечения вложенных функций
- [ ] 2.2.6 Обновление `ParseResult` для поддержки сложных структур
- [ ] 2.2.7 Тест: Парсинг .m файла с классами и вложенными функциями
- [ ] 2.2.8 Отметка выполнения в TODO.md (поставить ✅ в 2.2.1-2.2.6)
- [ ] 2.2.9 Коммит: `git commit -m "feat(parser): add support for classes and nested functions"`
- [ ] 2.2.10 Пуш: `git push origin master`
- [ ] 2.2.11 Суммаризация: Описание расширенного парсера
...
- [ ] 2.3.12 Переход к 2.4 и исполнению TODOs

### 2.4 Реализация таблицы символов (Symbol Table)
- [ ] **2.4.1 Анализ задачи**: Изучение LSP SymbolInformation, проектирование `SymbolTable` для хранения индекса символов. **Использовать**: context7 MCP для документации lsprotocol SymbolInformation, z_ai MCP для архитектуры Symbol Table
- [ ] 2.4.2 Создание `src/utils/symbol_table.py` с `SymbolTable`
- [ ] 2.4.3 Реализация добавления символов при открытии/изменении файлов
- [ ] 2.4.4 Реализация удаления символов при закрытии файлов
- [ ] 2.4.5 Реализация поиска символов по имени и URI
- [ ] 2.4.6 Интеграция с `MatlabParser` для пополнения Symbol Table
...
- [ ] 2.4.12 Переход к 2.5 и исполнению TODOs
...
- [ ] 3.5.4 Создание `tests/integration/test_code_action.py` для тестов быстрых исправлений
...
- [ ] 4.2.11 Суммаризация: Описание оптимизаций производительности
- [ ] 4.2.12 Переход к 4.3 и исполнению TODOs
...
- [ ] 4.3.6 Добавление хука mypy для type checking
- [ ] 4.3.7 Тест: Установка хуков `pre-commit install` и проверка коммита
- [ ] 4.3.8 Отметка выполнения в TODO.md (поставить ✅ в 4.3.1-4.3.6)
- [ ] 4.3.9 Коммит: `git commit -m "ci(pre-commit): add pre-commit hooks for code quality"`
- [ ] 4.3.10 Пуш: `git push origin master`
- [ ] 4.3.11 Суммаризация: Описание pre-commit хуков
- [ ] 4.3.12 Переход к 4.4 и исполнению TODOs

### 4.4 Создание CHANGELOG.md
- [ ] **4.4.1 Анализ задачи**: Изучение всех коммитов, планировка структуры CHANGELOG.md
- [ ] 4.4.2 Создание `CHANGELOG.md` с версией 0.1.0
- [ ] 4.4.3 Документирование Phase 1 изменений
- [ ] 4.4.4 Документирование Phase 2 изменений
- [ ] 4.4.5 Документирование Phase 3 изменений
- [ ] 4.4.6 Документирование Phase 4 изменений
- [ ] 4.4.7 Добавление секции "Breaking Changes" (если есть)
